...
function EditSocialSkill() {

    const [errors, setErrors] = useState({});
    const [socialSkill , setSocialSkill] = useState({name : '', pointSocial : 0, niveau: 'bas', dateAttribution:'', description: ''});
    const [loading,setLoading]= useState(false);
    const navigate = useNavigate();
    const {id} = useParams();
    
    const handleSaveSocialSkill = async () => {
        const data = {
        ...
        setLoading(true);
        await socialSkillService.updateSocialSkill(id,data);
        setLoading(false);
    ...
    const validateValues = (inputValues) => {
        ...
        } else if (!/^[a-zA-Z0-9\s]*$/.test(inputValues.name)) {
            errors.name = "Le nom ne doit contenir que des lettres, des chiffres et des espaces";
        }
        ...
        } else if (inputValues.description && !/^[a-zA-Z0-9\s]*$/.test(inputValues.description)) {
            errors.description = "La description ne doit contenir que des lettres, des chiffres et des espaces";
        }
    ...
    useEffect( () => {
       setLoading(true);
       const getSocialSkill = async () => {
          const data = await socialSkillService.getSocialSkillById(id);
          setSocialSkill(data);
       }
  ...
  return (
    ...
    <h1 className='text-3x1 my-4'>Mise à Juur d&apos;une compétence sociale</h1>
        ...
        <div className='my-4'>
            ...
            <form className="row g-3 needs-validation" noValidate >
                ...
                <div className="col-12">
                    <button type='submit' className="btn btn-primary" disabled={Object.keys(errors).length !== 0} onClick={handleSubmit} >Modifier</button>
                </div>
            </form>
        </div>
    </div>
...
export default EditSocialSkill