...
export default function Teste() {
  ...
  const imageUrls =
    conversations && conversations.image
      ? `${process.env.REACT_APP_BACKEND_URL}/${conversations.image}`
    ...
    const fetchMembres = async () => {
      try {
        if (selectedConversationId) {
          // Vérifiez si selectedConversationId est défini

          const response = await axios.get(
            `/messages/getMembres/${selectedConversationId}`
          );
  ...
  const visibleConversations = showMore
  ...
  const filteredMessages = messages.filter((message) => {
    ...
    return (
      ...
      (message.rang &&
        message.rang.toString().toLowerCase().includes(searchLowers))
    );
  });
            ...
            <div className="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
              <div className="card1 m-0">
                <div className="row no-gutters">
                  <div
                    className="col-xl-4 col-lg-4 col-md-4 col-sm-3 col-3"
                      ...
                      <div className="chat-search-box">
                        <div className="input-group">
                          ...
                          <Link
                            to="/addConversation"
                            title=""
                            style={{ paddingLeft: "5px", paddingTop: "9px" }}
                          >
                            ➕
                          </Link>
                        </div>
                          ...
                          (conversation) =>
                            ...
                            conversation.members.includes(user.id) && (
                              <li
                                ...
                                className="person"
                                data-chat={`person${conversation._id}`}
                                onClick={() => handleClick(conversation._id)}
                              >
                                <div className="user">
                                  {conversation.messages.length > 0 &&
                                  conversation.messages[0].repondeur
                                    .profileImage ? (
                                    <img
                                      src={`${process.env.REACT_APP_BACKEND_URL}/user/${conversation.messages[0]._id}/profile`}
                                      alt={
                                        conversation.messages[0].repondeur.name
                                      }
                                    />
                                  ) : (
                                    <img
                  ...
                  <div className="col-xl-8 col-lg-8 col-md-8 col-sm-9 col-9">
                    {selectedConversationDetails ? (
                        ...
                        <div className="selected-user">
                          <span>
                            ...
                            <div style={{ display: "flex", flexWrap: "wrap" }}>
                              {selectedConversationDetails.members.map(
                                (membre, index) => (
                                  <span
                                    key={index}
                                    style={{ margin: "0 10px" }}
                                  ...
                                  <img
                                    src={
                                      message.sender.profileImage
                                        ? `${process.env.REACT_APP_BACKEND_URL}/user/${message.sender._id}/profile`
                      ...
                      <div className="no-conversation-selected-message">
                        Veuillez sélectionner une conversation pour afficher le
                        chat.
                      </div>
                    )}
                  </div>
