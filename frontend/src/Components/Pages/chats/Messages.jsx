...
export default function Messages() {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      if (selectedConversationId) {
        try {
          const response = await axios.get(`/messages/getMessageById/${selectedConversationId}`);
  ...
  const handleMessageSend = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`/messages/conversation/${selectedConversationId}/send`, {
...
const handleDeleteConversation = async (conversationId) => {
  try {
      ...
      alert('Une erreur s\'est produite lors de la suppression de la  conversation.');
  }
};
  return (
    <>
 ...
 conversation.members.includes(user.id) &&  (
        ...
        <li key={conversation._id} onClick={() => handleClick(conversation._id)} 
            style={conversation._id === selectedConversation ? {backgroundColor: 'gainsboro'} : {}}> 
          <div className="usr-msg-details">
            ...
            <div className="usr-mg-info">
              ...
              <br/>
              <h6>{conversation.messages.length > 0 ? conversation.messages[conversation.messages.length - 1].content : "No messages"}</h6>
              <span className="posted_time">{conversation.messages.length > 0 ? moment(conversation.messages[conversation.messages.length - 1].createdAt).fromNow() : moment(conversation.createdAt).fromNow()}</span>
            </div>
                  ...
                  <div className="message-bar-head">
                    <div className="usr-msg-details">
                      ...
                      <div className="usr-mg-info">
                        ...
                        <p>{onlineUsers.includes(selectedConversationDetails?.name) ? 'En ligne' : 'Hors ligne'}</p>
                      </div>
                    </div>
                    <Link to={`/modifierConversation/${selectedConversationDetails?._id}`}title="">
  ...
  <path fillRule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z"/>
                      ...
                      <Link to={`/getUsers/${selectedConversationDetails?._id}`} title="">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-person-plus" viewBox="0 0 16 16">
                            <path d="M6 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6m2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0m4 8c0 1-1 1-1 1H1s-1 0-1-1 1-4 6-4 6 3 6 4m-1-.004c-.001-.246-.154-.986-.832-1.664C9.516 10.68 8.289 10 6 10s-3.516.68-4.168 1.332c-.678.678-.83 1.418-.832 1.664z"/>
                            <path fillRule="evenodd" d="M13.5 5a.5.5 0 0 1 .5.5V7h1.5a.5.5 0 0 1 0 1H14v1.5a.5.5 0 0 1-1 0V8h-1.5a.5.5 0 0 1 0-1H13V5.5a.5.5 0 0 1 .5-.5"/>
                          </svg>
  ...
  {messages.map((message, index) => (
    <div key={index} className={`main-message-box ${message.sender === user.id ? 'ta-right': 'ta-left'}`} style={message.sender === user.id ? {textAlign: 'right'} : {textAlign: 'left'}}>
    ...
    <a onClick={() => handleDelete(message._id)}>
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-x-octagon-fill" viewBox="0 0 16 16">
        ...
        <div className="message-inner-dt img-bx">
          <b>{message.sender!== user.id ? "Vous : " + message.content :  + message.content}</b>
      ...
      <div className="messg-usr-img">
        <img src={message.sender !== user.id ? "/assets/images/resources/m-img1.png" : "/assets/images/resources/m-img2.png"} alt="" />
      </div>
    </div>
  ))}
</div>
