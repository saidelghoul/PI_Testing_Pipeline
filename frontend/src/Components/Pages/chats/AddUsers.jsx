...
export default function AddUsers() {
  ...
  const handleUserClick = async (userId, userName) => {
    ...
    try {
      ...
      await axios.put(`/messages/updateMembres/${conversationId}`, { newMemberId: userId });
      // Rediriger vers la page de modification de la conversation
      navigate(`/message`);
    } catch (error) {
      console.error('Erreur lors de la mise à jour de la conversation : ', error);
      if (error.response && error.response.status === 400 && error.response.data.error === "L'utilisateur que vous essayez d'ajouter est déjà membre de cette conversation") {
        alert("L'utilisateur que vous essayez d'ajouter est déjà membre de cette conversation.");
      }
    }
  };
    ...
    <>
      <div className="suggestions full-width">
        ...
        <div className="suggestions-list">
          {users.map(userItem => (
            <div className="suggestion-usd" key={userItem._id} onClick={() => handleUserClick(userItem._id, userItem.name)}>
