...
export default function ListReplay() {
  ...
  const handleSubmitReplay = async (e) => {
    ...
    try {
      ...
      setNewReplayContent('');
      fetchReplays();  // Recharger les réponses
    } catch (error) {
  ...
  return (
            ...
            <div className="col-sm-10">
              {sujet && (
                <>
                  ...
                  <h1 className="media-heading" style={{ textDecoration: 'underline', fontWeight: 'bold' }}>
                  <i> 📚 Subject 📚: {sujet.title}</i></h1>
</div>
                  ...
                  <div className="media">
                    ...
                    <div className="media-body">
                      <h4 className="media-heading" style={{ textDecoration: 'underline', fontWeight: 'bold' }}>{sujet.Creator.name}</h4>
                      <p style={{  fontWeight: 'bold' }}>{sujet.content}</p>
                      <ul className="list-unstyled list-inline media-detail pull-left">
                        <br />
                        <li><i className="fa fa-calendar"></i>{moment(sujet.createdAt).format('DD/MM/YYYY')}</li>
                      </ul>
                    </div>
                  </div>
                  <br />
                  <hr />
                  <div className="media">
                    <h3> ╰┈➤ {replays.length} Replay{replays.length !== 1 && 's'}</h3>
                  </div>
                  <br />
                  <hr></hr>
                  {replays.map((replay, index) => (
                  <div key={replay._id} className="media">
    {replay.Creator.profileImage ? (
      <img src={`http://localhost:8000/user/${replay.Creator._id}/profile`} className="mr-3 rounded-circle" width="50" alt="User" />
    ) : (
      <img src="https://www.bootdey.com/img/Content/avatar/avatar3.png" alt="Default" />
    )}                         
    <div className="media-body">
      <h4 className="media" style={{ textDecoration: 'underline', fontWeight: 'bold' }}>{replay.Creator.name}</h4> 
      <p style={{ fontWeight: 'bold' }}>{replay.content}</p>
      <ul className="list-unstyled list-inline media-detail pull-left">
        <br />
        <li><i className="fa fa-calendar"></i>{moment(replay.createdAt).format('DD/MM/YYYY')}</li>
        <br></br>
        <hr></hr>
      </ul>
    </div>
    {index !== replays.length - 1 && <hr />}
              ...
              <form onSubmit={handleSubmitReplay}>
                <h3 className="pull-left">New Replay</h3>
                <br />
                <button type="submit" className="btn btn-normal pull-right" disabled={isSubmitting}>
                  {isSubmitting ? 'Submitting...' : 'Submit'}
                </button>
                <fieldset>
                  <div className="row">
                    <div className="col-sm-3 col-lg-2 hidden-xs">
                      <img className="img-responsive" src={imageUrl || "https://bootdey.com/img/Content/avatar/avatar1.png"} alt="" width="60" />
                    </div>
