...
async function add(req, res) {
  const { titre, contenu, datedeb, datefin, cap, prix, creator: userId } = req.body;
  ...
  try {
    ...
    if (new Date(datedeb) <= new Date()) {
      return res.status(400).json({ error: "La date de début doit être après la date actuelle" });
    }
...
async function createReservation(req, res) {
  try {
    ...
    res.status(500).json({ error: 'Erreur du serveur lors de la création de la réservation.' });
  }
};
...
module.exports = { getall, createReservation,getbyid, add, remove, update ,removeCondition};
